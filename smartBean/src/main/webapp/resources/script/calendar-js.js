...
$(document).ready(function() {
	...
	$("#save-newCalendar-button").on("click", function() {
		...
		if (newCalendarName.trim() !== "" && newCalendarOwner.trim() !== "") {
			...
			newCalendar.innerHTML = `<input type="checkbox" id="${newCalendarName}" class="calendar-checkbox-input" data-owner="${newCalendarOwner}">
	...
	$.ajax({
		...
		response.forEach(share => {
			...
			var newCalendar = document.createElement("div");
			newCalendar.className = "calendar-checkbox";
			newCalendar.innerHTML = `<input type="checkbox" id="${name}" class="calendar-checkbox-input" data-owner="${email}">
